...
def generate_pdf_latex(zal_key, context):
    ...
    Raises:
        FileNotFoundError: if the .tex.j2 template does not exist
        RuntimeError: if xelatex fails or produces no PDF
    """
    template_name = f"{zal_key}.tex.j2"
    template_path = os.path.join(LATEX_DIR, template_name)
